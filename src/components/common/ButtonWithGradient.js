...
export default Button = ({ style, onPress, disabled, bordered, gradient, icon, rightIcon, disableAllCaps = true, title }) => {
    const button = {
    ...
    return (
        ...
        <Clickable style={style} onPress={onPress} disabled={disabled} >
            <LinearGradient useAngle angle={45} start={{ x: 1, y: 1 }} end={{ x: 1, y: 0 }}
                colors={bordered ? ["transparent", "transparent"] : gradient || [Colors.buttonColor, Colors.buttonColor2]} style={[button, style]}>

                {icon ? <Image source={icon} style={{ marginEnd: 8 }} /> : null}
                <MyText style={textStyle} text={disableAllCaps ? title : title.toUpperCase()} />
                {rightIcon ? <Image source={rightIcon} style={{ marginStart: 8 }} /> : null}
